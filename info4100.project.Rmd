...
Currently, this early warning system is not predicting the students who will dropout with sufficient precision and recall for its F1 score to be F1>0/7. Initially, our F1 score was only 0.09, but over the course of creating new features with higher correlations, we were able to increase our F1 score significantly to become 0.2722513. The click data in the model did a very poor job at predicting wether or not an assignment was submitted or not, this lead to an intially low F1 score, however in our final model, we mostly omitted click data in order to improve the recall and precision of our model. Our final model mainly uses features that focus on data gained from the assignments and submission times such as the earliest time they submitted before the dealing, how many points were taken off their assignments, and the mean grade they recieved in the course. We would not recommend implementing the system currently until we are able to gain more click data that can be utilized to generate more accuarte features that we can base our model on since the 13 click features we discarded did a very poor job at prediciting submissions of the students. However an important thing to note is that information about assignment submission, time taken, and grades received do a good job at predicting the submisisons of the students and should definitely be used in a model used for an early warning system. 
%###############################################
